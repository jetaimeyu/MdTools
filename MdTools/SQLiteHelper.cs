...
namespace MdTools
{
    ...
    public static class SQLiteHelper
    {
        ...
        //public static SQLiteConnection connString { set;get{return QQinit() };
        public static string  QQinit()
        {
            var path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "db", "data.db");
            if (File.Exists(path) == false) File.Create(path);
             string connString = $@"Data Source={path}; Initial Catalog=sqlite;Integrated Security = True; Max Pool Size=100; Password=;";
            return connString;
            //try
            //{
            ...
            //catch (Exception ex)
            //{
            //    throw new Exception("未找到可用的数据模型！" + ex.Message);
            //}
        }

        // Application.StartupPath
     //   public static string LocalDbConnectionString = ConfigurationManager.ConnectionStrings["localdb"].ConnectionString;
        //Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "db", "data.db")
        public static string LocalDbConnectionString = $@"Data Source={Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "db", "data.db")}; Initial Catalog=sqlite;Integrated Security = True; Max Pool Size=100; Password=;";
